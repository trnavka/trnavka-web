...
if ( !class_exists('afgFlickr') ) {
    ...
    class afgFlickr {
        ...
        function request ($command, $args = array(), $nocache = false)
        {
            ...
            } elseif (!empty($_SESSION['afgFlickr_auth_token'])) {
                $args = array_merge($args, array("auth_token" => $_SESSION['afgFlickr_auth_token']));
            }
            ksort($args);
            $auth_sig = "";
        ...
        function setProxy ($server, $port) {
            // Sets the proxy for all afgFlickr calls.
        ...
        function auth ($perms = "read", $remember_uri = true) {
            ...
            if (empty($_SESSION['afgFlickr_auth_token']) && empty($this->token)) {
                if ( $remember_uri === true ) {
                    $_SESSION['afgFlickr_auth_redirect'] = $_SERVER['REQUEST_URI'];
                } elseif ( $remember_uri !== false ) {
                    $_SESSION['afgFlickr_auth_redirect'] = $remember_uri;
                }
        ...
        To use the afgFlickr::call method, pass a string containing the API method you want
        ...
        *******************************/
        ...
        function auth_getToken ($frob) {
            ...
            $_SESSION['afgFlickr_auth_token'] = $this->parsed_response['auth']['token'];
            return $this->parsed_response ? $this->parsed_response['auth'] : false;
        }
...
if ( !class_exists('afgFlickr_pager') ) {
    class afgFlickr_pager {
        var $afgFlickr, $per_page, $method, $args, $results, $global_afgFlickr;
        var $total = null, $page = 0, $pages = null, $photos, $_extra = null;


        function __construct($afgFlickr, $method = null, $args = null, $per_page = 30) {
        ...
        function set_afgFlickr($afgFlickr) {
            if ( is_a($afgFlickr, 'afgFlickr') ) {
                $this->afgFlickr = $afgFlickr;
                if ( $this->afgFlickr->cache ) {
        ...
        function load($page) {
            ...
            if ( $this->afgFlickr->cache ) {
                ...
                if ( floor($min/500) == floor($max/500) ) {
                    ...
                    $this->results = $this->afgFlickr->call($this->method, $this->args);
                    if ( $this->results ) {
