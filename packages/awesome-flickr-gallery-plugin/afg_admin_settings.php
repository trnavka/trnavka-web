...
function afg_admin_menu() {
    add_menu_page('Flickr Gallery', 'Flickr Gallery', 'publish_pages', 'afg_plugin_page', 'afg_admin_html_page', '', 898);
    $afg_main_page = add_submenu_page('afg_plugin_page', 'Default Settings | Flickr Gallery', 'Default Settings', 'publish_pages', 'afg_plugin_page', 'afg_admin_html_page');
    $afg_add_page = add_submenu_page('afg_plugin_page', 'Add Gallery | Flickr Gallery', 'Add Gallery', 'edit_posts', 'afg_add_gallery_page', 'afg_add_gallery');
    $afg_saved_page = add_submenu_page('afg_plugin_page', 'Saved Galleries | Flickr Gallery', 'Saved Galleries', 'edit_posts', 'afg_view_edit_galleries_page', 'afg_view_delete_galleries');
    $afg_edit_page = add_submenu_page('afg_plugin_page', 'Edit Galleries | Flickr Gallery', 'Edit Galleries', 'edit_posts', 'afg_edit_galleries_page', 'afg_edit_galleries');
    $afg_advanced_page = add_submenu_page('afg_plugin_page', 'Advanced Settings | Flickr Gallery', 'Advanced Settings', 'edit_pages', 'afg_advanced_page', 'afg_advanced_settings_page');
    ...
    global $pf, $custom_size_err_msg;
        ...
        if (isset($_POST['submit']) && $_POST['submit'] == 'Delete Cached Galleries') {
            ...
            if (get_option('afg_api_secret') && !get_option('afg_flickr_token')) {
                echo "<div class='updated'><p><strong>Click \"Grant Access\" button to authorize Flickr Gallery to access your private photos from Flickr.</strong></p></div>";
            }
        }
        create_afgFlickr_obj();
    }
...
<?php
    ...
    $message = "Just insert the code <strong><font color='steelblue'>[AFG_gallery]</font></strong> in any of your posts or pages to display the Flickr Gallery.
        <br /><p style='text-align:center'><i>-- OR --</i></p>You can create a new Flickr Gallery with different settings on page <a href='" . get_admin_url() . "admin.php?page=afg_add_gallery_page'>Add Galleries.";
